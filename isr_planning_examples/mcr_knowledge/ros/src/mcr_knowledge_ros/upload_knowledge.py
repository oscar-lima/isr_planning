...
this dictionary is then sent por doing individual service calls via rosplan_update_knowledge() function
"""
    ...
    for each in pddl_instances_as_dictionary:
        for element in each:
            ...
            try:
                ...
                rosplan_update_knowledge(0, knowledge.get('instance_type'), knowledge.get('instance_name').split('--')[1], '', [])
            except:
...
def upload_facts(pddl_init_vector):
    ...
    for each in pddl_init_vector[1:]:
        ...
        rosplan_update_knowledge(1, '', '', each[0], key_values)
...
def upload_goals(pddl_goal_vector):
    ...
    for each in pddl_goal_vector[1][1:]:
        key_values = compute_key_values(each)
        rosplan_update_knowledge(1, '', '', each[0], key_values, update_type='ADD_GOAL')
...
def upload_knowledge(pddl_vector):
    ...
    if len(pddl_vector) == 1:
        rospy.logdebug(pddl_cathegory + " knowledge is empty, while uploading pddl knowledge")
    else:
