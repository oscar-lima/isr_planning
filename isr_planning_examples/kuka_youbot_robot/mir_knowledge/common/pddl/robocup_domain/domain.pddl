...
(define (domain robocup_at_work)
 ...
 (:predicates
	...
	; the center of a robot ?r is in front of the object ?o
	(aligned_with ?r - robot ?o - object)

	; the robot knows how much it has to move sideways to be aligned with objects
	(alignment_distances_available ?l - location)
...
; to a folded position, then navigate
 ...
 ; align the robot with an object
 (:action align_base_with_object
     :parameters (?r - robot ?target ?previous - object ?l - location ?g - gripper)
     :precondition 	(and 	(at ?r ?l)
     						(gripper_is_free ?g)
                      		(on ?target ?l)
                      		(alignment_distances_available ?l)
                      		(aligned_with ?r ?previous)
                   	)
     :effect (and  	(aligned_with ?r ?target)
     				(not (aligned_with ?r ?previous))
     				(not (perceived ?previous))
                   	(increase (total-cost) 1)
             )
 )

 ; perceive the location and calculate how much does the base needs to shift
 ; sideways to be in front of the object
 (:action compute_shift_relative_distances
     :parameters (?l - location ?o - object ?r - robot ?g - gripper)
     :precondition 	(and 	(at ?r ?l)
     						          (gripper_is_free ?g)
                      		(on ?o ?l)
                   	)
     :effect (and  	(alignment_distances_available ?l)
                   	(increase (total-cost) 1)
             )
 )
